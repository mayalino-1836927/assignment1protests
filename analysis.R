...
if ((str_detect("(")) == FALSE) {
  high_level_purpose <- str_dup(purpose, c(1, ))
} else {
...
own_function <- function(purposes) {
  attendee_count <- length(purpose)
  return("The number of attendees for the issue of ", purposes, " are: ", attendee_count)
}